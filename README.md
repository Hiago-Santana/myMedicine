# My Medicine

O projeto visa prover ao usuário uma maneira simples e rápida de organizar seus medicamentos de acordo com as necessidades,
receitadas por um médico. Este aplicativo não visa substituir o trabalho de um profissional, mas sim auxiliar e complementar
a atuação. Organize suas medicações ao longo dos dias, semanas, meses e anos, e não se esqueça mais de administrá-los.

# 📱 Download do APK

Para instalar e testar o aplicativo no Android, acesse o APK disponível no Google Drive:

👉 **[Clique aqui para acessar a pasta pública com o arquivo APK](https://drive.google.com/drive/folders/1JGZy9J2rWbRxBZzSykqTycZ_PN7aW2ct)**

# Tecnologias utilizadas

- *Vite*: `v7.1.7`
- *Vue*: `3.5.22`
- *Tailwindcss*: `4.1.15`

# Como rodar

### Requisitos para executar localmente

- ***Node.js (versão recomendada: 18+)***

Para rodar, 

1. Certifique-se de que o **Node.js** ou **Yarn** está instalado.
2. Clone este repositório:

```git clone <https://github.com/Hiago-Santana/myMedicine.git>```

3. Entrar no diretório

```cd my-medicine>```

4. Instalar dependências

```npm install```

5. Rodar o app

```npm run dev```

### A tela inicial aparecerá assim
<img src="img/TELA 1.png" alt="Passo 1" width="650" height="350"/>

### Ao clicar em novo medicamento, esta será a próxima tela
<img src="img/TELA 2.png" alt="Passo 2" width="650" height="350"/>

### Ao preencher todos os campos corretamente, a opção de 'adicionar medicamento' será liberada. Clique nela.
<img src="img/TELA 3.png" alt="Passo 3" width="650" height="350"/>

### Pronto! Seu medicamento estará salvo, conforme suas prefêrencias
<img src="img/TELA 4.png" alt="Passo 4" width="650" height="350"/>

### Nestas telas, é possível observar e administrar os dados preenchidos, se necessário
<img src="img/TELA 5.png" alt="Passo 5" width="450" height="300"/>
<img src="img/TELA 6.png" alt="Passo 6" width="450" height="300"/>

# Funcionalidades

Ao entrar na tela principal da aplicação, é possível ver os campos "Horários do dia", "Meus medicamentos" e "Novo medicamento".
 - Horários do dia: Neste campo, é possível ver os horários agendados para determinadas medicações predefinidas e devidamente preenchidas
 pelo próprio usuário, para o dia em questão.
 - Meus medicamentos: Aqui são listados todos os medicamentos administrados, no geral. Temos informações de dosagem, dia específico, horário, etc.
 - Novo medicamento: A área é responsável por adicionar uma nova receita ao calendário, com uma medicação que será preenchida pelo usuário.

# Como contribuir

Clone o projeto com ```git clone /mnt/data/README.md```

01 - Acesse o diretório do projeto:

```cd my-medicine```

02 - Crie uma nova branch para sua contribuição:

```git checkout -b minha-contribuicao```


03 - Faça suas alterações e commit:

```git add .```
```git commit -m "Descrição das alterações"```


04 - Envie sua branch:

```git push origin minha-contribuicao```


05 - Abra um Pull Request no GitHub, descrevendo claramente o que foi alterado ou adicionado.

# Integrantes 

``Alexandre dos Santos Camaragibe``
``Bruna Nayara M. S. dos Anjos``
``Giulles Santos``
``Hiago D. G. S. Santana``
``Karina Silva de Souza``
